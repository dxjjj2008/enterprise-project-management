<template>
  <div class="board-page">
    ...
    <div class="board-body" v-loading="loading">
      <!-- 无任务提示 -->
      <el-empty v-if="!hasVisibleTasks" description="暂无任务" :image-size="120" role="alert" aria-live="assertive" aria-label="当前没有任务">
      ...
      <!-- 看板列 -->
      <div class="board-columns" v-else role="listbox" aria-label="任务列表" aria-live="polite">
        <div
        ...
        <div class="task-detail">
          ...
          <div class="detail-section">
            ...
            <div class="subtask-list">
              ...
              <el-empty v-if="!currentTask.subtasks?.length" description="暂无子任务" :image-size="60" role="alert" aria-live="assertive" aria-label="当前没有子任务" />
            </div>
          </div>
          ...
          <div class="detail-section">
            ...
            <div class="comment-list">
              ...
              <el-empty v-if="!currentTask.comments?.length" description="暂无评论" :image-size="60" role="alert" aria-live="assertive" aria-label="当前没有评论" />
            </div>
          </div>
        </div>
      </template>
...
// 看板列配置
const columns = ref([
  { id: 'todo', title: '待办', status: 'todo', color: '#FA8C16', bgColor: '#FFF7E6' },
      ...
      try {
        if (isEdit.value) {
          ...
          const taskData = {
            ...
            // 转换日期格式为 ISO 字符串
            due_date: taskForm.value.dueDate ? new Date(taskForm.value.dueDate).toISOString() : null
          }
          await createTask(currentProject.value, taskData)
          // 重新加载任务列表
          await loadTasks()
...
const loadTasks = async () => {
  loading.value = true
  try {
    const response = await getTasks(currentProject.value)
    // API 返回格式: { success, message, data: [...], total, page, page_size }
    if (response && response.data) {
      ...
      tasks.value = response.data.map(task => ({
        ...
        assignee: task.assigned_to ? { id: 1, name: task.assigned_to, avatar: '' } : null,
